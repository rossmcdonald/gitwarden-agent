...
if [[ "$(readlink /proc/1/exe)" == */systemd ]]; then
    ...
    rm -f /lib/systemd/system/gitwarden-agent.service
else
    which update-rc.d &>/dev/null && update-rc.d -f gitwarden-agent remove
    which chkconfig &>/dev/null && chkconfig --del gitwarden-agent
